...
namespace TerrificNet.Thtml.Test.Asserts
{
	...
	internal static class HandlebarsExpressionAssert
	{
		public static void AssertExpression(MustacheExpression expected, MustacheExpression result)
		{
			...
			var parentExpression = expected as ParentExpression;
			...
			var self = expected as SelfExpression;
			...
			{
				Assert.IsType<ParentExpression>(result);
				AssertAccessExpression(parentExpression, result as ParentExpression);
			}
			else if (self != null)
			{
				Assert.IsType<SelfExpression>(result);
			}
		...
		{
			Assert.Equal(expected.Name, result.Name);
			AssertAccessExpression(expected, result);
		}

		private static void AssertAccessExpression(AccessExpression expected, AccessExpression result)
		{
