...
namespace TerrificNet.Thtml.Binding
{
	...
	{
		private static readonly MethodInfo GetValueFromDictionaryMethod = typeof(DelegateBuilder).GetTypeInfo().GetMethod(nameof(GetValueFromDictionary));
		...
		public static Func<object, object> Dictionary(Type modelType, string key)
		{
			...
			var itemType = modelType.GetTypeInfo().GetGenericArguments()[1];
